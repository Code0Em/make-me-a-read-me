...
// *TASK 10: Create an object (builtWith) containing all build options (as the keys) and the markdown code for their corresponding badges (as the values). Create an empty array (to hold the badges markdown code) (badges). Create a function with a for of loop which iterates through the userAnswers array (n.b. Inquirer checkbox prompt returns an array of user’s selected answers). Within for of loop, add a for in loop which iterates through the builtWith object, within this add an if statement which checks if the builtWith key matches the value from the userAnswers array. If it does, push the corresponding value from the builtWith object up to the badges array. Outside of both loops, use join() method on badges array which concatenates all the elements and returns a new string (ergo removes commas). Return this string and run this function within the Build section of the makeReadme function.
